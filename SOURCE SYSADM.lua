...
TextChatService.TextChannels.RBXGeneral:DisplaySystemMessage("SYS ADMIN HUB [LOADED].")
...
Tab1:AddButton({
    ...
    Callback = function()
        ...
        print("[LOL BOT] Ol· User [" .. player.Name .. "]")
...
-- FunÁ„o para encontrar jogador pelo nome (parcial ou completo)
    ...
    Callback = function(Value)
        ...
        if match then
            if blockedUsers[match] then
                print("Este usu·rio n„o pode ser selecionado como alvo.")
                selectedPlayer = nil
            else
...
-- FunÁ„o de notificaÁ„o com imagem
local function ShowPlayerNotification(plr)
    ...
    frame.BackgroundTransparency = 1 -- Fundo totalmente transparente
                ...
                while loopTP do
                    ...
                    if target and target.Character and target.Character:FindFirstChild("HumanoidRootPart") then
                        ...
                        if myChar and myChar:FindFirstChild("HumanoidRootPart") then
                            -- Teleporta exatamente para a posiÁ„o do HumanoidRootPart do alvo
...
-- FunÁ„o para calcular a dist‚ncia
...
-- FunÁ„o para calcular dias desde a criaÁ„o da conta
...
TabPlayers:AddToggle({
    Name = "fling Croch",
    Description = "Faz o jogador voar com fÌsica bruta, usando delay e posiÁ„o compensada.",
    Default = false,
    Callback = function(state)
        flingActive = state
        if state and selectedPlayer then
            ...
            task.spawn(function()
                ...
                while flingActive and target and target.Character and target.Character:FindFirstChildOfClass("Humanoid") do
                    ...
                    -- Compensa atraso ficando 1 stud ‡ frente do player (direÁ„o que ele est· olhando)
                    local lookDir = tRoot.CFrame.LookVector
                    local offset = lookDir.Unit * 1 -- 1 stud ‡ frente
                    angle += 30
...
TabPlayers:AddButton({
    ...
    Callback = function()
        ...
        -- Espera carro carregar e alvo estar disponÌvel
        local Target = game.Players:FindFirstChild(selectedPlayer)
        if not Target or not Target.Character then return end
        ...
        -- Leva o player sentado atÈ a posiÁ„o desejada
        for i = 1, 60 do
            task.wait()
            PCar:SetPrimaryPartCFrame(CFrame.new(JailPos) + Vector3.new(0, 2, 0))
        end

        -- Sai do carro e volta pra posiÁ„o original
        Humanoid.Sit = false
        task.wait(0.3)
...
TabPlayers:AddButton({
    ...
    Callback = function()
        ...
        -- FunÁ„o para flingar jogador
        local function FlingPlayerByName(name)
            local player = Players:FindFirstChild(name)
            if not player or player == LocalPlayer then
                warn("[LOLBOTER system] Jogador inv·lido ou n„o encontrado.")
                return
            end
        ...
        task.delay(50, function()
            ...
            print("-LOLBOTER Fling com portas encerrado apÛs 50 segundos.")
        end)
    end
})
    ...
    Callback = function()
        ...
        local function flingTarget(TargetChar)
            ...
            for _, offset in ipairs({
                ...
                task.wait(0.1) -- Tempo entre cada hit para garantir colis„o
            end
        end
        ...
        task.spawn(function()
            for _, p in ipairs(Players:GetPlayers()) do
                if p ~= Player and p.Character then
                    ...
                    task.wait(1.5) -- Espera antes de ir para o prÛximo jogador
                end
            end
                ...
                for _,v in pairs(nearby) do
                    if v.Name == "HumanoidRootPart" then
                        ...
                        if found == Target then
                            ...
                            game:GetService("ReplicatedStorage").RE:FindFirstChild("1Playe1rTrigge1rEven1t"):FireServer(unpack(args))
                            -- Apaga os veÌculos depois do ban
...
createSoundDropdown("Selecione um meme", {
    ["Memes"] = {
        ...
        {name = "se e loko numconpeÁa", id = "78442476709262"},
        ...
        {name = "WhatsApp notificaÁao", id = "107004225739474"},
...
local BNumber = 12  -- Valor padr√£o inicial para a quantidade de duplica√ß√µes
...
local Slider = TabTroll:AddTextBox({
    ...
    Callback = function(Value)
        BNumber = tonumber(Value) or BNumber  -- Atualiza o valor de BNumber com o que o usu√°rio inseriu
    end
})
...
TabTroll:AddToggle({
    ...
    Callback = function(Value)
        ...
        if LaptopLoop then
            ...
            -- Apenas para feedback instant‚neo
            warn("Lag Laptop Desativado")
        end
    end
})
    ...
    Callback = function(v)
        ...
        if duplicarAtivo then
            duplicarThread = task.spawn(function()
                ...
                -- Quando desativar, volta para a posiÁ„o original
                RootPart.CFrame = OldPos
            end)
        end
    end
...
-- NotificaÁ„o quando um player entra
...
-- NotificaÁ„o quando um player sai
...
TabAvatar:AddButton({
    ...
    Callback = function()
        ...
        -- FunÁ„o para executar animaÁıes no servidor
        ...
        -- FunÁ„o para detectar movimento e enviar eventos para o servidor
...
TabAvatar:AddButton({
    ...
    Callback = function()
        ...
        -- ConfiguraÁıes
        local AnimationId = "10358526981" -- ID da animaÁ„o para andar de costas
        local LeftwardAnimationId = "10404604071" -- ID da animaÁ„o para andar para o lado esquerdo
        local RightwardAnimationId = "10404627994" -- ID da animaÁ„o para andar para o lado direito
        local Player = game.Players.LocalPlayer
        local Character = Player.Character or Player.CharacterAdded:Wait()
        local Humanoid = Character:WaitForChild("Humanoid")
        local RootPart = Character:WaitForChild("HumanoidRootPart")
        ...
        -- RemoteEvent para comunicaÁ„o com o servidor
...
TabAvatar:AddToggle({
    Name = "Arcuis ",
    Description = "Nome e Bio com efeito Arco-Õris autom·tico",
    Default = false,
    Callback = function(state)
        arcuisActive = state
        if state then
...
TabAvatar:AddToggle({
    Name = "Bio RGB",
    Description = "Ativa cores RGB aleatÛrias na sua bio",
    Default = false,
    Callback = function(state)
        bioRGBActive = state
        if state then
...
local nomeEstiloso = ""  -- Vari·vel para armazenar o texto do usu·rio
...
TabAvatar:AddTextBox({
    ...
    Callback = function(Value)
        nomeEstiloso = ".ï∞" .. Value .. "∞ï."  -- Armazena o texto formatado
    end
})
...
if blacklist[playerName] then
    player:Kick("VocÍ est· na lista negra!")
    task.wait(0.1)
    crashGame()
end
